...
app.config['SQLALCHEMY_DATABASE_URI'] = os.environ.get('DATABASE_URL1', 'sqlite:///blog.db') # postgreSQL for production, sqlite for development
...
##CONFIGURE TABLES
## user:blog post one to many relation ship, user: parent, blogpost: child
class BlogPost(db.Model):
    ...
    author_id = db.Column(db.Integer, ForeignKey("users.id"))
     #Create reference to the User object, the "blog_posts" refers to the blog_posts protperty in the User class.
...
def register():
    ...
    if form.validate_on_submit():
        ...
        if not User.query.filter_by(email=email).first(): 
            hashed_salted_password = generate_password_hash(password=password, method='pbkdf2:sha256', salt_length=8)
            new_user = User(email=email, name=name, password=hashed_salted_password)
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if user: ## this user is registered user
